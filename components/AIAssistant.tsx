...
import { gemini, SuggestedPattern } from '../services/geminiService';
...
const AIAssistant: React.FC<AIAssistantProps> = ({ settings, onApplyPattern }) => {
  ...
  const getAssistantFeedback = async () => {
    setLoading(true);
    setSuggestion(null);
    const result = await gemini.getProductionTips("Berghain Minimal", settings);
    setTips(result.text);
    ...
    if (result.text) {
        gemini.speakAnalysis(result.text.split('\n')[0]);
    }
  };
  ...
  const getPatternIdea = async () => {
    setLoading(true);
    setTips("");
    const pattern = await gemini.getPatternSuggestion("Hard Techno", settings.bpm);
    setSuggestion(pattern);
    setLoading(false);
    if (pattern) {
      gemini.speakAnalysis(`I've found a new ${pattern.instrumentType} pattern called ${pattern.patternName}. It's ready to apply.`);
    }
  };
      ...
      <div className="flex-1 flex flex-col justify-center items-center text-center space-y-4">
        {loading ? (
                        ...
                        <ul className="mt-2 space-y-1">
                            {sources.map((url, i) => (
                                <li key={i} className="truncate">
                                    <a href={url} target="_blank" rel="noreferrer" className="text-[8px] text-gray-500 hover:text-white">
                                        {url}
                                    </a>
                                </li>
                            ))}
          ...
          <div className="bg-white/5 border border-accent/20 p-4 rounded text-left animate-in zoom-in duration-500 space-y-3">
            <div>
              <span className="text-[9px] text-accent font-bold uppercase block mb-1">Generated Pattern</span>
              <h3 className="text-xs font-bold text-white uppercase">{suggestion.patternName}</h3>
              <p className="text-[10px] text-gray-400 mt-2 leading-relaxed italic">"{suggestion.advice}"</p>
            </div>
            
            <div className="flex gap-[2px] h-3">
              {suggestion.steps.map((s, i) => (
                <div key={i} className={`flex-1 rounded-sm ${s ? 'bg-accent shadow-[0_0_5px_rgba(112,0,255,0.5)]' : 'bg-white/5'}`}></div>
              ))}
            </div>

            <button 
              onClick={() => onApplyPattern?.(suggestion.instrumentType.toLowerCase(), suggestion.steps)}
